...
public class SearchBox : TemplatedControl
{
    ...
    public static readonly StyledProperty<Func<string, CancellationToken, Task<IEnumerable>>?>
    ...
    private void TextBox_PropertyChanged(object? sender, AvaloniaPropertyChangedEventArgs e)
    {
        if (e.Property == TextBox.TextProperty)
        {
            ...
            if (!string.IsNullOrWhiteSpace(SearchText)) _searchTimer?.Start();
        }
    }
    ...
    private async Task PerformSearch()
    {
        ...
        try
        {
            ...
            if (!token.IsCancellationRequested && Collection != null && results != null)
                ...
                {
                    Collection.Clear();
                    foreach (var item in results) Collection.Add(item);
                });
